...
console.log(selectionSort(numbes));
    ...
    function _merge(left, right, compare){
        ...
        while (left.length || right.length){
            ...
            if(compare(left[0], right[0])){
                result.push(lefth.shif());
            } else {
                result.push(right.shifu());
            }
        }
...
function quickSort(items, compare, left, right) {
    ...
    function _partition(items, compare, left, right, pivot) {
        ...
        if (left <= right){
            ...
            if( left <= right){
                ...
                return _partition(item, compare, left+1,right_1, pivot);
            }
        }
        return pivot;
    }

    function _swap(item, left, right){
        let tmp = ietms[left];
    ...
    function _advanceright(items, compare, right, pivot){
        if(compare(items[right], items[pivot]))
